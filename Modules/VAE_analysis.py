...
#N args- N rows
def plot_galaxies(*args,dimensions='2d',cmap='gray'):
    ...
    for row in range(len(args)):
        for col in range(n):
            if dimensions=='2d':
                ...
                ax.imshow(args[row][col].squeeze(),cmap=cmap,vmax=I_max)
            else:
...
def calculate_gradient(image_true,image_pred,reconstruction_loss_function,labels=None):
...
def present_reconstruction(model,images,labels=None,reconstruction_loss_function=None,number_of_images_to_consider=8,dimensions='2d',resid=False, grads=False,abs_grads=True,cmap='gray'):
    #Images selection
    print(number_of_images_to_consider)
    images_for_reconst=images
    decoded_to_reconstruct=model.predict(images_for_reconst, batch_size=len(images))
    plot_galaxies(images_for_reconst[:number_of_images_to_consider],decoded_to_reconstruct[:number_of_images_to_consider],dimensions=dimensions,cmap=cmap)
    if grads:
        if (reconstruction_loss_function is not None) and (labels is not None):
          #Gradients in form of (len(images),64,64,1)
          gradients=calculate_gradient(images_for_reconst,decoded_to_reconstruct.astype('double'),reconstruction_loss_function,labels.astype('double'))
          if abs_grads:
            ...
            norm_gradients=np.array([norm_gradients[i]/norm_gradients[i].flatten().max() for i in range(len(norm_gradients))])
        plot_galaxies(norm_gradients[:number_of_images_to_consider],dimensions=dimensions,cmap=cmap)
    if resid:
      ...
      plot_galaxies(residuals[:number_of_images_to_consider],dimensions=dimensions,cmap=cmap)
...
def Show_latent_distr(models,x_test_labeled,reconst_loss_func=tf.keras.losses.binary_crossentropy,regul_loss_func=None,loss_stats=True):
